...
void add(std::vector<std::pair<std::string, int>>& box, std::string name, int value)
{
    bool found = false;
    for (std::pair<std::string, int>& element : box)
    {
    ...
    if (!found)
    {
        box.push_back(std::pair<std::string, int>{name, value});
    }
}


void remove(std::vector<std::pair<std::string, int>>& box, std::string name)
{
...
int main() {
    std::string line;
    int part1_sum {};
    std::vector<std::vector<std::pair<std::string, int>>> box {};
    for (int i {}; i <= 256; i++)
    {
        box.push_back({});
    }
    while (std::getline(std::cin, line)) 
    {
        for (size_t pos {}; line.find(',', pos) != std::string::npos; pos = line.find(',', pos+1)) 
        {
            ...
            if (word.find('=') != std::string::npos)
            {
                ...
                add(box[get_hash(name)], name, value);
            } else  // contains '-'
            {
                size_t split = word.find('-');
                std::string name = word.substr(0, split);
    ...
    for (size_t box_nr {}; box_nr < box.size(); box_nr++)
    {
        ...
        {
            part2_sum += (box_nr+1) * (slot_nr+1) * box[box_nr][slot_nr].second;
        }
    }
